...
// The /analyze call runs three agents server-side in sequence.
const ANALYZE_SUB_AGENTS = ['Evidence Retrieval Agent', 'Gap Analysis Agent', 'Question Agent'];
...
function inferAgent(path: string): string | undefined {
  ...
  if (path.includes('/candidates/') && path.endsWith('/analyze')) return 'Gap Analysis Agent';
...
function emitTrace(
  type: 'start' | 'finish',
  payload: { path: string; method?: string; agent?: string; status?: number; error?: string; candidateId?: string; jobId?: string }
) {
  if (typeof window === 'undefined') return;
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  // The analyze call runs Evidence Retrieval → Gap Analysis → Question Agent
  // server-side. Emit traces for all three so they show up in the workflow.
  if (isAnalyze && agent) {
    for (const subAgent of ANALYZE_SUB_AGENTS) {
  ...
  if (isAnalyze) {
    for (const subAgent of ANALYZE_SUB_AGENTS) {
      emitTrace('finish', { path, method, agent: subAgent, status: response.status, candidateId, jobId });
    }
  } else {
    emitTrace('finish', { path, method, agent, status: response.status, candidateId, jobId });
  }
  return payload as T;
}
